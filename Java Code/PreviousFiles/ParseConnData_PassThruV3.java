...
public class ParseConnData_PassThruV3 {
   ...
   public static void main(String[] args) {
      ...
      try {
         ...
         if(graph.isCyclic())
            ...
            bufWriter2.write("srcIP,dstIP,count,dur,bytes,hops\n");
            ...
            for(int i=0;i<newSrcDstList.size();i++) {
               ...
               bufWriter2.write(key+","+connectionCount+","+totDuration+","+totBytes+","+hopCount+"\n");
            }
            bufWriter2.close();
         }
      ...
      System.out.println("==========================================================");
      System.out.println("Producing Final Graph: ");
      for(int i=0;i<newSrcDstList.size();i++) {
         String key             = newSrcDstList.get(i);
         int    connectionCount = 0;
         double totDuration     = 0.0;
         ...
         if(finalGraph.containsKey(temp[0])) {
            vSrc = finalGraph.get(temp[0]);
         } else {
            // new src vertice
            //System.out.println("New src vertice created: " + temp[0]);
            vSrc = new Vertice(temp[0]);
            finalGraph.put(temp[0], vSrc);
         }            
         
         if(finalGraph.containsKey(temp[1])) {
            vDst = finalGraph.get(temp[1]);
         } else {
            // new dst vertice
            //System.out.println("New dst vertice created: " + temp[1]);
            vDst = new Vertice(temp[1]);
            finalGraph.put(temp[1], vDst);
         }
         ...
         Edge e = new Edge(vSrc, vDst, connectionCount, totDuration, totBytes);
      ...
      try {
         ...
         // file header
         bufWriter.write("srcIP,dstIP,count,dur,bytes,hops,end");
         ...
         for(String key : finalGraph.keySet()) {   
            ...
            if(v.inDegree() == 0) {
               ...
               Integer hop = 0;
               iteratePath(v.getEdgesOut(), hop, bufWriter);
            }
         }
         bufWriter.write("\n");
         bufWriter.close();
   ...
   public static void iteratePath(List<Edge> edges, Integer hop, BufferedWriter bufWriter) {
      ...
      if(edges.size() == 0) {
         ...
         for(Edge e : edges) {
            ...
            System.out.print(" ["+e.getCount()+","+e.getDuration()+","+e.getBytes()+"] -> " + e.getDst().toString() + "("+hop+")");
            iteratePath(e.getDst().getEdgesOut(), hop, bufWriter);
         }
      }
   }
}